...
def train(dirpath,outdir,epoch = 50,data_augment = 0):
    ...
    if data_augment == 0:
        ...
        signalgen = AugmentationGenerator(X_train, Y_train, batch_size,wlen,num_classes,data_augment,epoch)
        ...
        history = model.fit_generator(signalgen.flow(),steps_per_epoch=signalgen.numbatch(),
                                      validation_data=batchgen.flow(),validation_steps=batchgen.numbatch(),
                                      epochs=epoch,callbacks=[modelCheckpoint])
